...
function displayBooks() {
    ...
    for(const item of myLibrary) {
        ...
        if (item.pages) pages.textContent = `Pages: ${item.pages}`;
        ...
        buttonSection.appendChild(read);
        ...
        if (item.read) {
            read.classList.add('read-card');
        } else {
            read.classList.add('unread-card');
        }
...
container.addEventListener('click', function(e) {
    ...
    if (e.target.classList.contains('read')) {
        const parentCard = e.target.closest('.card');
        const id = parentCard.dataset.number;
